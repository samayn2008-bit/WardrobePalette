...
public class WardrobePaletteApp extends JFrame {
    ...
    public WardrobePaletteApp() {
        ...
        preview.setBorder(new LineBorder(PAPER, 1, true));
    ...
    private void loadImage(File file) {
        try {
            ...
            if (image == null) {
                throw new IllegalArgumentException("That file is not a supported image.");
            }
    ...
    private void showRankings(Color shirt) {
        ...
        pantList.removeAll(); 
        double max = scored.getFirst().score;
    ...
    private JPanel pantCard(ScoredPant ranked, boolean best, double max) {
        ...
        if (best) {
            backgroundColor = new Color(58, 74, 99);
            borderColor = DENIM;
        } else {
    ...
    private static Color averageColor(BufferedImage image) {
        BufferedImage sample = scale(image, 100, 100); 
        long r = 0, g = 0, b = 0;
        for (int y = 0; y < 100; y++) {
            for (int x = 0; x < 100; x++) {
                Color pixel = new Color(sample.getRGB(x, y));
        ...
        int averageRed = (int) (r / 10000);
        int averageGreen = (int) (g / 10000);
        int averageBlue = (int) (b / 10000);
